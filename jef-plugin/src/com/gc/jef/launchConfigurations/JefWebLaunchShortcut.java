...
public class JefWebLaunchShortcut extends JavaApplicationLaunchShortcut {
	...
	protected ILaunchConfigurationType getConfigurationType() {
		return getLaunchManager().getLaunchConfigurationType(PluginHelper.JEF_WEB_APPLICATION);
	}
...
//		}
	...
	protected ILaunchConfiguration findLaunchConfiguration(ILaunchConfigurationType configType,IJavaProject jp) {
		List<ILaunchConfiguration> candidateConfigs = new ArrayList<ILaunchConfiguration>();
		try {
			ILaunchConfiguration configs[] = DebugPlugin.getDefault().getLaunchManager().getLaunchConfigurations(configType);
			for (int i = 0; i < configs.length; i++) {
				ILaunchConfiguration config = configs[i];
				if (config.getAttribute(IJavaLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAME, "").equals("jef.database.JefClassLoader") && config.getAttribute(IJavaLaunchConfigurationConstants.ATTR_PROJECT_NAME, "").equals(jp.getElementName()))
	...
	protected ILaunchConfiguration createConfiguration(IJavaProject jp) {
		...
		try {
			...
			wc.setAttribute(IJavaLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAME, "jef.database.JefClassLoader");
//			wc.setMappedResources(new IResource[] { type.getUnderlyingResource() });
	...
	protected void launch(String mode, IJavaProject jp) {
		ILaunchConfiguration config = findLaunchConfiguration(getConfigurationType(),jp);
		if (config == null)
			config = createConfiguration(jp); // TYPE可能是外部工程的
		if (config != null)
